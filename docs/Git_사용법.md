...
```text
main  ←  개발·테스트 완료 후 dev 에서 병합 (최종 버전)
 │
 dev  ←  개발 중 통합 브랜치. 기능 브랜치 PR 은 여기로 모은다
 │
...
| 브랜치 | 역할 |
|--------|------|
| **main** | 개발 최종 버전. 모든 개발·테스트 완료 후 **dev → main** 병합 |
| **dev** | 개발 중 사용. 기능 브랜치들이 PR로 병합되는  integr 브랜치 |
| **개발 브랜치** | 이슈별로 생성. 개별 작업 후 **dev** 로 PR |
...
| From | To | 시기 | Merge 방식 |
|------|-----|------|------------|
| `feat/#N` 등 | **dev** | 기능 단위 완료 시 | **Squash and Merge** |
| **dev** | **main** | 통합 테스트·데모 준비 완료 시 | 팀 합의 후 (보호 규칙 준수) |
