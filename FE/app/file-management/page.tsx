...
const createMockFile = (id: number, name: string, type: string, status: string, scope: string, roles: string[], checkCount: number | null, date: string, size: string, tags: string[]) => ({
  id: `mock-doc-${id}`, docId: `mock_doc_${String(id).padStart(3, '0')}`, name: `[목업] ${name}`, type, status, scope, roles, checkCount,
  date, size, updatedAt: `2026.${date.replace(' ', ' ')}`, uploader: '김세원 (신입 구성원)', description: '화면 확인을 위해 입력한 목업 문서입니다.', tags,
});

const MOCK_FILES = [
  createMockFile(1, '행사운영가이드.pdf', 'pdf', 'READY', 'WORKSPACE', ['NEW_HIRE', 'MEMBER'], 42, '08.16 18:21', '5.8MB', ['행사', '운영', '가이드']),
  createMockFile(2, '예산안_v7.xlsx', 'excel', 'PROCESSING', 'ROLE_BASED', ['NEW_HIRE'], null, '08.16 18:18', '2.4MB', ['예산', '결재']),
  createMockFile(3, '거래처_연락망.xlsx', 'excel', 'FAILED', 'WORKSPACE', ['MEMBER'], 0, '08.16 18:13', '1.6MB', ['거래처', '연락망']),
  createMockFile(4, '브랜드가이드.pdf', 'pdf', 'PENDING', 'ROLE_BASED', ['NEW_HIRE'], null, '08.16 18:11', '8.2MB', ['브랜드', '디자인']),
  createMockFile(5, '주간회의양식.pptx', 'pptx', 'READY', 'WORKSPACE', ['ALL'], 18, '08.16 17:44', '3.1MB', ['회의', '양식']),
  createMockFile(6, '월간성과보고서.xlsx', 'excel', 'READY', 'ROLE_BASED', ['MANAGER', 'MEMBER'], 27, '08.15 16:30', '4.7MB', ['성과', '보고']),
  createMockFile(7, '콘텐츠검수체크리스트.pdf', 'pdf', 'READY', 'WORKSPACE', ['NEW_HIRE', 'MEMBER'], 31, '08.14 14:05', '1.1MB', ['콘텐츠', '검수']),
  createMockFile(8, '캠페인일정표.xlsx', 'excel', 'PENDING', 'ROLE_BASED', ['NEW_HIRE', 'MANAGER'], null, '08.13 10:22', '980KB', ['캠페인', '일정']),
];
...
export default function FileManagementPage() {
  ...
  const filteredFiles = files.filter((file) => {
    const statusMatches = selectedFilter === 'all' || file.status === selectedFilter;
    const searchMatches = file.name.toLowerCase().includes(searchQuery.toLowerCase());
        ...
        <div className={styles.contentArea}>
          ...
          <div className={styles.leftSection}>
                ...
                {filteredFiles.map((file, idx) => (
                  ...
                  >
                    ...
                    <div className={styles.colStatus}>
                      <span
                        className={`${styles.statusBadge} ${getStatusClass(file.status)}`}
                      >
                        {getDisplayLabel(file.status)}
                      </span>
                    </div>
                    <div className={styles.colScope}>{getDisplayLabel(file.scope)}</div>
                    <div className={styles.colRoles}>
                      {file.roles.map((role, i) => (
                        <span key={i} className={styles.roleBadge}>{getDisplayLabel(role)}</span>
                      ))}
                    </div>
                    ...
                    <div className={styles.colAction}>
                      {file.status === 'FAILED' ? (
                        <button className={styles.retryBtn} title="문서 재처리" aria-label={`${file.name} 재처리`} onClick={(event) => {
                          event.stopPropagation();
                          setSelectedFile(idx);
                          setIsReprocessConfirmOpen(true);
                        }}><RotateCcw size={16} /></button>
                      ) : (
                        <button className={styles.moreBtn} title="파일 작업 열기" aria-label={`${file.name} 작업 열기`} onClick={(event) => {
                          event.stopPropagation();
                          setSelectedFile(idx);
            ...
            <div className={styles.footerNote}>
              <span>업로드할 때 공개 범위와 허용 역할을 설정할 수 있으며, 문서를 다시 처리할 수도 있습니다.</span>
            </div>
          </div>
            ...
            {currentFile && (
              <div className={styles.detailsCard}>
                <div className={styles.detailsHeader}>
                  ...
                  <span
                    className={`${styles.detailsStatus} ${getStatusClass(currentFile.status)}`}
                  >
                ...
                <div className={styles.detailsMeta}>
                  ...
                  <div className={styles.metaRow}>
                    ...
                    <span
                      className={`${styles.metaValueStatus} ${getStatusClass(currentFile.status)}`}
                    >
                      {getDisplayLabel(currentFile.status)}
                    </span>
                  </div>
                  <div className={styles.metaRow}>
                    ...
                    <span className={styles.metaValue}>{getDisplayLabel(currentFile.scope)}</span>
                  </div>
                  <div className={styles.metaRow}>
          ...
          footer={
            <>
              ...
              <ModalPrimaryButton
                ...
                onClick={() => run('file-management-0', '목업 파일을 추가했습니다.', () => {
                  const mockFile = createMockFile(Date.now(), `${uploadFileName || '새 문서'}.pdf`, 'pdf', 'PENDING', uploadScope, uploadRoles, null, '08.19 14:30', '1.0MB', ['목업']);
                  mockFile.description = uploadDescription || '화면 확인을 위해 추가한 목업 문서입니다.';
        ...
        >
          ...
          <div className={styles.formGroup}>
            ...
            >
              ...
              <option value="ROLE_BASED">역할별 공개</option>
            ...
            <div className={styles.sources}>
              ...
              <div className={styles.sourceButtons}>
                <button className={styles.sourceBtn}>
                  <FileText size={14} /> {currentFile.name} · 3쪽
                </button>
                <button className={styles.sourceBtn}>
                  <FileText size={14} /> {currentFile.name} · 5쪽
                </button>
              </div>
            </div>
      ...
      {isReprocessConfirmOpen && currentFile && (
        ...
        >
          <div className={styles.confirmHeader}>
            <p>재처리를 시작하면 상태가 대기 중 → 처리 중으로 변경되며,</p>
      ...
      {isDeleteConfirmOpen && currentFile && (
          ...
          footer={
            <>
              ...
              <ModalDangerButton
                ...
                onClick={() => {
                  run('file-management-2', '목업 문서를 삭제했습니다.', () => {
        ...
        >
          ...
          <p className={styles.deleteSubtext}>
            이 문서는 휴지통으로 이동되며, 인공지능 답변에 담긴 내용은 더 이상 제공되지 않습니다.
          </p>
            ...
            <div className={styles.sidePanelBody}>
              ...
              <div className={styles.timeline}>
                <div className={styles.timelineItem}>
                  <div className={styles.timelineIcon} style={{ backgroundColor: '#287456' }}><Check size={14} /></div>
                ...
                <div className={styles.timelineItem}>
                  <div className={styles.timelineIcon} style={{ backgroundColor: '#0765FC' }}><Settings size={14} /></div>
                  <div className={styles.timelineContent}>
                ...
                <div className={styles.timelineItem}>
                  <div className={styles.timelineIcon} style={{ backgroundColor: '#287456' }}><Check size={14} /></div>
                  <div className={styles.timelineContent}>
                    <p className={styles.timelineTitle}>준비 완료로 전환</p>
                    <p className={styles.timelineSubtext}>문서가 준비 완료 상태로 전환되었습니다.</p>
                    <p className={styles.timelineTime}>08.16 18:21:01</p>
                  </div>
                </div>
              </div>
