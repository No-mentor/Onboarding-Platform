  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof AuthError) {
        ...
        } else if (error.isEmailNotVerified() || (error.isForbidden() && error.message.includes('인증'))) {
          setErrors({});
          setShowVerifyPrompt(true);
        } else if (error.isForbidden()) {
          if (error.message && (error.message.includes('이메일 인증') || error.message.includes('인증'))) {
            setErrors({});
            setShowVerifyPrompt(true);
          } else {
